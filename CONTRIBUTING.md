# 🛠️ Contributing to Jirnyak/stenograph
...
> Technology Foundation: `TypeScript / JavaScript / Web Audio / Web Speech API`
...
Stenograph High-Speed Chord Parser & Real-Time Subsystem is engineered for maximum performance, deterministic state transitions, and zero computational slop. All contributions must respect existing subsystem boundaries and data flows:

```mermaid
graph LR
    A[Hardware Keystroke Burst] --> B[Sub-20ms Chord Window Aggregator]
    B -->|Chord Token| C[Dictionary Prefix Trie Lookup]
    C -->|Orthographic Text Translation| D[Real-Time Output Buffer]
```
...
### 1. Sub-20ms Chord Resolution
* **Formal Requirement**: Keypress grouping into chords must evaluate temporal windows under 20ms.
...
### 2. Trie Lookup Complexity
* **Formal Requirement**: Orthographic translation lookups must execute in O(K) time where K is stroke length.
* **Verification Protocol**: Automated unit test assertion + mathematical boundary check.
* **Failure Mode**: Immediate build rejection; PR cannot be approved without meeting this invariant.
...
* Primary Runtime: `TypeScript / JavaScript / Web Audio / Web Speech API`
...
```typescript
// ✅ CORRECT: O(K) Prefix Trie Lookup
export function lookupChord(trie: TrieNode, strokes: string[]): string | null {
    let curr: TrieNode | undefined = trie;
    for (const stroke of strokes) {
        curr = curr.children[stroke];
        if (!curr) return null;
    }
